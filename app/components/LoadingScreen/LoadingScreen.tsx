    ...
    <div ref={refs.loadingRef} className="min-h-screen flex items-center justify-center bg-black text-white fixed inset-0 z-50 loading-container">
      <div className="text-center">
        <h1 className="font-semibold mb-4">
          ...
          <span ref={refs.staticTextRef} className="text-lg sm:text-4xl md:text-4xl"> {staticText}</span>
          <span ref={refs.typingTextRef} className="text-lg sm:text-4xl md:text-4xl inline-block w-48 sm:w-80 md:w-96 text-left">
            {displayText}
            <span ref={refs.cursorRef} className={`ml-1 ${showCursor ? 'opacity-100' : 'opacity-0'} transition-opacity duration-100`}>
              <div className="inline-block w-2 h-5 sm:w-4 sm:h-8 bg-white"></div>
            </span>
          </span>
        </h1>
